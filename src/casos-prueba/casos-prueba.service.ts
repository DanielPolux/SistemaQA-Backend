...
export class CasosPruebaService {
  ...
  async findAll(query: QueryCasoPruebaDto): Promise<PaginatedResponseDto<any>> {
    ...
    const qb = this.casosRepo
      ...
      .leftJoin('c.requerimiento', 'req').addSelect(['req.codigo'])
      .leftJoin('c.asignado', 'a').addSelect(['a.nombre', 'a.apellido'])
      .leftJoin('c.creador', 'u').addSelect(['u.nombre', 'u.apellido'])
      .loadRelationCountAndMap('c.totalDefectos', 'c.id', 'def')
      .skip(skip)
      .take(porPagina)
    ...
    if (query.asignadoA) qb.andWhere('c.asignadoA = :aa', { aa: query.asignadoA });
    if (query.busqueda) {
      qb.andWhere('(c.titulo ILIKE :b OR c.codigo ILIKE :b)', { b: `%${query.busqueda}%` });
    }
    ...
    const datos = casos.map((c) => ({
      ...
      requerimientoCodigo: c.requerimiento?.codigo ?? null,
      asignadoANombre: c.asignado ? `${c.asignado.nombre} ${c.asignado.apellido}` : null,
  ...
  async findOne(id: number): Promise<any> {
    const c = await this.casosRepo.findOne({
      where: { id },
      relations: ['proyecto', 'requerimiento', 'asignado', 'creador'],
    });
